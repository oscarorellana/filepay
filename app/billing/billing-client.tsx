'use client'

import { useEffect, useMemo, useState } from 'react'
import { useSearchParams } from 'next/navigation'
import { createClient } from '@supabase/supabase-js'

const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
)

type SubRow = {
  plan: string | null
  status: string | null
  cancel_at_period_end: boolean | null
  current_period_end: string | null // timestamptz ISO
  updated_at: string | null
}

type LinkRow = {
  code: string
  file_path: string | null
  expires_at: string | null
  paid: boolean | null
  paid_at: string | null
  created_at: string | null
  created_by_user_id: string | null
}

function formatDateShort(iso: string | null | undefined) {
  if (!iso) return ''
  const d = new Date(iso)
  if (Number.isNaN(d.getTime())) return ''
  return d.toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })
}

function formatDateTimeShort(iso: string | null | undefined) {
  if (!iso) return ''
  const d = new Date(iso)
  if (Number.isNaN(d.getTime())) return ''
  return d.toLocaleString(undefined, {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
    hour: '2-digit',
    minute: '2-digit',
  })
}

export default function BillingPage() {
  const sp = useSearchParams()
  const tab = (sp?.get('tab') ?? 'billing').trim().toLowerCase()
  const showLinks = tab === 'links'

  const [email, setEmail] = useState<string | null>(null)
  const [userId, setUserId] = useState<string | null>(null)

  const [busy, setBusy] = useState(false)
  const [msg, setMsg] = useState('')
  const [sub, setSub] = useState<SubRow | null>(null)

  // Links state
  const [linksBusy, setLinksBusy] = useState(false)
  const [linksMsg, setLinksMsg] = useState('')
  const [links, setLinks] = useState<LinkRow[]>([])

  const isSignedIn = Boolean(email)
  const isPro = useMemo(() => sub?.plan === 'pro' && sub?.status === 'active', [sub])
  const cancelScheduled = useMemo(() => Boolean(sub?.cancel_at_period_end), [sub])
  const periodEndLabel = useMemo(() => formatDateShort(sub?.current_period_end) || '', [sub])

  const billingTimeline = useMemo(() => {
    if (!isSignedIn) return null
    if (!isPro) return null

    if (cancelScheduled) {
      return periodEndLabel
        ? { label: `Ends on ${periodEndLabel}`, tone: 'warn' as const }
        : { label: 'End date will appear shortly. Hit Refresh.', tone: 'warn' as const }
    }

    return periodEndLabel
      ? { label: `Renews on ${periodEndLabel}`, tone: 'pro' as const }
      : { label: 'Next billing date will appear shortly. Hit Refresh.', tone: 'pro' as const }
  }, [isSignedIn, isPro, cancelScheduled, periodEndLabel])

  const badge = useMemo(() => {
    if (!isSignedIn) return { label: 'SIGN IN', tone: 'free' as const }
    if (!isPro) return { label: 'FREE', tone: 'free' as const }
    return cancelScheduled
      ? { label: 'PRO · Cancel scheduled', tone: 'warn' as const }
      : { label: 'PRO · Active', tone: 'pro' as const }
  }, [isSignedIn, isPro, cancelScheduled])

  const origin = useMemo(() => {
    if (typeof window === 'undefined') return ''
    return window.location.origin
  }, [])

  async function loadFromDb() {
    setMsg('')
    setBusy(true)
    try {
      const { data: u } = await supabase.auth.getUser()
      const user = u.user ?? null

      setEmail(user?.email ?? null)
      setUserId(user?.id ?? null)

      if (!user?.id) {
        setSub(null)
        return
      }

      const { data: row, error } = await supabase
        .from('subscriptions')
        .select('plan,status,cancel_at_period_end,current_period_end,updated_at')
        .eq('user_id', user.id)
        .maybeSingle()

      if (error) throw new Error(error.message)
      setSub((row ?? null) as SubRow | null)
    } catch (e: any) {
      setMsg(e?.message ?? 'Something went wrong')
    } finally {
      setBusy(false)
    }
  }

  async function syncFromStripe({ quiet = false }: { quiet?: boolean } = {}) {
    if (!quiet) {
      setMsg('')
      setBusy(true)
    }
    try {
      const { data } = await supabase.auth.getSession()
      const token = data.session?.access_token
      if (!token) {
        if (!quiet) setMsg('Please sign in first.')
        return
      }

      const res = await fetch('/api/pro/sync', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      })

      const json = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(json?.error || json?.message || 'Sync failed')

      // Reload DB row after sync
      await loadFromDb()
    } catch (e: any) {
      if (!quiet) setMsg(e?.message ?? 'Something went wrong')
    } finally {
      if (!quiet) setBusy(false)
    }
  }

  async function openPortal() {
    setMsg('')
    setBusy(true)
    try {
      const { data } = await supabase.auth.getSession()
      const token = data.session?.access_token
      if (!token) {
        setMsg('Please sign in first.')
        return
      }

      const res = await fetch('/api/pro/portal', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      })

      const json = await res.json().catch(() => ({}))
      if (!res.ok || !json?.url) throw new Error(json?.error || 'Failed to open subscription settings')

      window.location.href = json.url
    } catch (e: any) {
      setMsg(e?.message ?? 'Something went wrong')
    } finally {
      setBusy(false)
    }
  }

  async function loadLinks() {
    setLinksMsg('')
    setLinksBusy(true)

    try {
      const { data: u } = await supabase.auth.getUser()
      const user = u.user ?? null
      setEmail(user?.email ?? null)
      setUserId(user?.id ?? null)

      const { data: sess } = await supabase.auth.getSession()
      const token = sess.session?.access_token
      if (!token) {
        setLinks([])
        setLinksMsg('Please sign in to see your links.')
        return
      }

      const res = await fetch('/api/my-links', {
        method: 'GET',
        headers: { Authorization: `Bearer ${token}` },
      })

      const json = (await res.json().catch(() => ({}))) as { links?: LinkRow[]; error?: string }

      if (!res.ok) {
        throw new Error(json?.error || 'Failed to load links')
      }

      const rows = Array.isArray(json.links) ? json.links.filter((r) => r?.code) : []
      setLinks(rows)
      if (!rows.length) setLinksMsg('No links yet. Create one from Home.')
    } catch (e: any) {
      setLinksMsg(e?.message ?? 'Could not load links')
      setLinks([])
    } finally {
      setLinksBusy(false)
    }
  }

  async function copy(text: string) {
    try {
      await navigator.clipboard.writeText(text)
      setLinksMsg('Copied ✅')
      window.setTimeout(() => setLinksMsg(''), 1200)
    } catch {
      setLinksMsg('Copy failed (browser blocked clipboard)')
    }
  }

  useEffect(() => {
    ;(async () => {
      await loadFromDb()
      await syncFromStripe({ quiet: true })
      if ((sp?.get('tab') ?? 'billing').trim().toLowerCase() === 'links') {
        await loadLinks()
      }
    })()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  useEffect(() => {
    if (showLinks) loadLinks()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [showLinks])

  return (
    <main style={styles.page}>
      <div style={styles.container}>
        <div style={styles.card}>
          <div style={styles.header}>
            <div>
              <h1 style={styles.h1}>{showLinks ? 'My links' : 'Manage subscription'}</h1>
              <p style={styles.p}>
                {showLinks
                  ? 'View your created links, expiry, and purchase status.'
                  : 'Update your plan, payment method, or cancel anytime.'}
              </p>

              {/* ✅ FIX hydration: no <div> inside <p> */}
              {/* Signed-in header */}
          <div style={{ ...styles.p, opacity: 0.8, display: 'flex', alignItems: 'center', gap: 8 }}>
            {email ? (
  <>
    <span>
      Signed in as <b>{email}</b>
    </span>

    {sub?.plan === 'pro' && (
        <span
          style={{
            fontSize: 11,
            padding: '2px 8px',
            borderRadius: 999,
            background: '#111',
            color: '#fff',
            fontWeight: 600,
            letterSpacing: 0.3,
          }}
        >
          PRO
        </span>
      )}
    </>
  ) : (
    <span>Not signed in</span>
  )}
</div>

              <div
                style={{
                  ...styles.badge,
                  ...(badge.tone === 'pro'
                    ? styles.badgePro
                    : badge.tone === 'warn'
                      ? styles.badgeWarn
                      : styles.badgeFree),
                }}
              >
                {badge.label}
              </div>

              <div style={styles.tabsRow}>
                <a
                  href="/billing?tab=billing"
                  style={{ ...styles.tabBtn, ...(showLinks ? {} : styles.tabBtnActive) }}
                >
                  Billing
                </a>
                <a
                  href="/billing?tab=links"
                  style={{ ...styles.tabBtn, ...(showLinks ? styles.tabBtnActive : {}) }}
                >
                  My links
                </a>
              </div>
            </div>

            <a href="/" style={styles.linkBtn}>
              ← Back
            </a>
          </div>

          {showLinks ? (
            <>
              <div style={styles.linksTopRow}>
                <button
                  type="button"
                  onClick={loadLinks}
                  style={{ ...styles.secondaryBtn, opacity: linksBusy ? 0.75 : 1 }}
                  disabled={linksBusy}
                >
                  {linksBusy ? 'Refreshing…' : 'Refresh links'}
                </button>

                <a href="/" style={styles.secondaryLink}>
                  Create a new link
                </a>
              </div>

              {linksMsg && <div style={styles.note}>{linksMsg}</div>}

              <div style={styles.linksWrap}>
                {links.length === 0 ? (
                  <div style={styles.smallHint}>{email ? 'No links yet.' : 'Sign in to view your links.'}</div>
                ) : (
                  links.map((l) => {
                    const url = origin
                      ? `${origin}/dl/${encodeURIComponent(l.code)}`
                      : `/dl/${encodeURIComponent(l.code)}`
                    const expires = formatDateTimeShort(l.expires_at)
                    const paidAt = formatDateTimeShort(l.paid_at)
                    const createdAt = formatDateTimeShort(l.created_at)

                    const statusPill = l.paid
                      ? { label: 'Paid', style: styles.pillPaid }
                      : { label: 'Unpaid', style: styles.pillUnpaid }

                    return (
                      <div key={l.code} style={styles.linkItem}>
                        <div style={styles.linkTop}>
                          <div style={styles.linkCodeRow}>
                            <div style={styles.linkCode}>{l.code}</div>
                            <span style={{ ...styles.pill, ...statusPill.style }}>{statusPill.label}</span>
                          </div>

                          <div style={styles.linkActions}>
                            <button type="button" onClick={() => copy(url)} style={styles.smallBtn}>
                              Copy link
                            </button>
                            <a href={`/dl/${encodeURIComponent(l.code)}`} style={styles.smallBtnLink}>
                              Open
                            </a>
                          </div>
                        </div>

                        <div style={styles.linkMetaGrid}>
                          <div style={styles.metaCell}>
                            <div style={styles.metaLabel}>Created</div>
                            <div style={styles.metaValue}>{createdAt || '—'}</div>
                          </div>

                          <div style={styles.metaCell}>
                            <div style={styles.metaLabel}>Expires</div>
                            <div style={styles.metaValue}>{expires || '—'}</div>
                          </div>

                          <div style={styles.metaCell}>
                            <div style={styles.metaLabel}>Paid at</div>
                            <div style={styles.metaValue}>{paidAt || '—'}</div>
                          </div>
                        </div>

                        <div style={styles.fileRow}>
                          <div style={styles.metaLabel}>File</div>
                          <div style={styles.filePath}>{l.file_path || '—'}</div>
                        </div>
                      </div>
                    )
                  })
                )}
              </div>
            </>
          ) : (
            <>
              <div style={styles.summary}>
                <div style={styles.summaryRow}>
                  <div style={styles.summaryLabel}>Plan</div>
                  <div style={styles.summaryValue}>{isSignedIn ? (isPro ? 'Pro' : 'Free') : '—'}</div>
                </div>

                {isSignedIn && isPro && (
                  <>
                    <div style={styles.summaryRow}>
                      <div style={styles.summaryLabel}>Cancel scheduled</div>
                      <div style={styles.summaryValue}>{cancelScheduled ? 'Yes' : 'No'}</div>
                    </div>

                    <div style={styles.summaryRow}>
                      <div style={styles.summaryLabel}>{cancelScheduled ? 'Ends' : 'Renews'}</div>
                      <div style={styles.summaryValue}>
                        {billingTimeline?.label ?? 'Next billing date will appear shortly. Hit Refresh.'}
                      </div>
                    </div>
                  </>
                )}

                {isSignedIn && !isPro && (
                  <div style={styles.smallHint}>
                    Want unlimited links without paying per link? Check <b>Pricing</b> to upgrade.
                  </div>
                )}
              </div>

              {/* PRIMARY ACTION */}
{isSignedIn && isPro ? (
  <button
    type="button"
    onClick={openPortal}
    style={{ ...styles.primaryBtn, opacity: busy ? 0.75 : 1 }}
    disabled={busy}
  >
    {busy ? 'Opening…' : 'Manage subscription'}
  </button>
) : (
  <a
    href="/pricing"
    style={{
      ...styles.primaryBtn,
      display: 'inline-block',
      textAlign: 'center',
      textDecoration: 'none',
    }}
  >
    Upgrade to Pro
  </a>
)}

              <div style={styles.secondaryRow}>
                <button
                  type="button"
                  onClick={() => syncFromStripe({ quiet: false })}
                  style={{ ...styles.secondaryBtn, opacity: busy ? 0.75 : 1 }}
                  disabled={busy}
                >
                  {busy ? 'Refreshing…' : 'Refresh status'}
                </button>

                {!isPro && (
                  <a href="/pricing" style={styles.secondaryLink}>
                    View pricing
                  </a>
                )}
              </div>

              {msg && <div style={styles.note}>{msg}</div>}

              <div style={styles.smallHint}>
                After cancelling or changing billing, it can take a moment to update. Use <b>Refresh status</b>.
              </div>
            </>
          )}
        </div>
      </div>
    </main>
  )
}

const styles: Record<string, React.CSSProperties> = {
  page: {
    minHeight: '100svh',
    padding: 24,
    fontFamily: 'ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial',
    background:
      'radial-gradient(1200px 600px at 18% 0%, rgba(124,58,237,0.22), transparent 60%), radial-gradient(900px 500px at 90% 10%, rgba(59,130,246,0.18), transparent 55%), #07070a',
    color: 'white',
  },
  container: { maxWidth: 980, margin: '0 auto' },

  card: {
    borderRadius: 18,
    border: '1px solid rgba(255,255,255,0.10)',
    background: 'rgba(255,255,255,0.04)',
    boxShadow: '0 18px 40px rgba(0,0,0,0.35)',
    padding: 16,
    display: 'grid',
    gap: 14,
  },

  header: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' },
  h1: { margin: 0, fontSize: 28, fontWeight: 950, letterSpacing: -0.2 },
  p: { margin: '6px 0 0', opacity: 0.8, fontSize: 13, lineHeight: 1.35 },

  tabsRow: { marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' },
  tabBtn: {
    display: 'inline-block',
    padding: '8px 10px',
    borderRadius: 999,
    border: '1px solid rgba(255,255,255,0.14)',
    background: 'rgba(255,255,255,0.06)',
    color: 'white',
    fontWeight: 900,
    fontSize: 12,
    textDecoration: 'none',
  },
  tabBtnActive: {
    background: 'rgba(124,58,237,0.22)',
    border: '1px solid rgba(124,58,237,0.35)',
  },

  badge: {
    marginTop: 10,
    display: 'inline-flex',
    alignItems: 'center',
    padding: '6px 10px',
    borderRadius: 999,
    fontSize: 12,
    fontWeight: 900,
    border: '1px solid rgba(255,255,255,0.14)',
    width: 'fit-content',
  },
  badgePro: { background: 'rgba(124,58,237,0.22)' },
  badgeWarn: { background: 'rgba(245,158,11,0.18)' },
  badgeFree: { background: 'rgba(255,255,255,0.06)' },

  linkBtn: {
    display: 'inline-block',
    padding: '8px 10px',
    borderRadius: 10,
    border: '1px solid rgba(255,255,255,0.14)',
    background: 'rgba(255,255,255,0.06)',
    color: 'white',
    fontWeight: 800,
    fontSize: 12,
    textDecoration: 'none',
    height: 'fit-content',
  },

  summary: {
    padding: 12,
    borderRadius: 16,
    border: '1px solid rgba(255,255,255,0.10)',
    background: 'rgba(0,0,0,0.18)',
    display: 'grid',
    gap: 10,
  },
  summaryRow: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'baseline' },
  summaryLabel: { opacity: 0.75, fontSize: 12 },
  summaryValue: { fontWeight: 950, fontSize: 12, textAlign: 'right' },

  primaryBtn: {
    padding: '12px 14px',
    borderRadius: 14,
    border: '1px solid rgba(255,255,255,0.16)',
    background: 'linear-gradient(135deg, rgba(255,255,255,0.95), rgba(255,255,255,0.78))',
    color: '#0b0b10',
    fontWeight: 950,
    cursor: 'pointer',
    boxShadow: '0 12px 28px rgba(0,0,0,0.35)',
  },

  secondaryRow: {
    display: 'flex',
    justifyContent: 'space-between',
    gap: 12,
    alignItems: 'center',
    flexWrap: 'wrap',
  },
  secondaryBtn: {
    padding: '10px 12px',
    borderRadius: 12,
    border: '1px solid rgba(255,255,255,0.14)',
    background: 'rgba(255,255,255,0.06)',
    color: 'white',
    fontWeight: 900,
    cursor: 'pointer',
    fontSize: 12,
  },
  secondaryLink: {
    display: 'inline-block',
    padding: '10px 12px',
    borderRadius: 12,
    border: '1px solid rgba(255,255,255,0.14)',
    background: 'rgba(255,255,255,0.04)',
    color: 'white',
    fontWeight: 900,
    fontSize: 12,
    textDecoration: 'none',
  },

  note: {
    padding: 12,
    borderRadius: 14,
    border: '1px solid rgba(255,255,255,0.10)',
    background: 'rgba(0,0,0,0.22)',
    fontSize: 12,
    opacity: 0.9,
    whiteSpace: 'pre-wrap',
  },

  smallHint: { marginTop: 2, opacity: 0.65, fontSize: 12, lineHeight: 1.35 },

  linksTopRow: { display: 'flex', gap: 10, justifyContent: 'space-between', flexWrap: 'wrap', alignItems: 'center' },
  linksWrap: { display: 'grid', gap: 10 },

  linkItem: {
    borderRadius: 16,
    border: '1px solid rgba(255,255,255,0.10)',
    background: 'rgba(0,0,0,0.18)',
    padding: 12,
    display: 'grid',
    gap: 10,
  },
  linkTop: { display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-start', flexWrap: 'wrap' },
  linkCodeRow: { display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' },
  linkCode: { fontWeight: 950, letterSpacing: 0.2, fontSize: 14 },

  pill: {
    fontSize: 11,
    fontWeight: 900,
    padding: '4px 8px',
    borderRadius: 999,
    border: '1px solid rgba(255,255,255,0.12)',
    background: 'rgba(255,255,255,0.06)',
  },
  pillPaid: { background: 'rgba(34,197,94,0.12)' },
  pillUnpaid: { background: 'rgba(245,158,11,0.12)' },

  linkActions: { display: 'flex', gap: 8, flexWrap: 'wrap' },
  smallBtn: {
    padding: '8px 10px',
    borderRadius: 12,
    border: '1px solid rgba(255,255,255,0.14)',
    background: 'rgba(255,255,255,0.06)',
    color: 'white',
    fontWeight: 900,
    fontSize: 12,
    cursor: 'pointer',
  },
  smallBtnLink: {
    padding: '8px 10px',
    borderRadius: 12,
    border: '1px solid rgba(255,255,255,0.14)',
    background: 'rgba(255,255,255,0.06)',
    color: 'white',
    fontWeight: 900,
    fontSize: 12,
    textDecoration: 'none',
  },

  linkMetaGrid: {
    display: 'grid',
    gridTemplateColumns: 'repeat(3, minmax(0, 1fr))',
    gap: 10,
  },
  metaCell: { display: 'grid', gap: 4 },
  metaLabel: { opacity: 0.7, fontSize: 11 },
  metaValue: { fontWeight: 900, fontSize: 12, wordBreak: 'break-word' },

  fileRow: {
    display: 'grid',
    gap: 4,
    borderTop: '1px solid rgba(255,255,255,0.08)',
    paddingTop: 10,
  },
  filePath: {
    fontSize: 12,
    opacity: 0.9,
    wordBreak: 'break-word',
    fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace',
  },
}